...
def add_note():
    ...
    note = data.Note.Note(title=title, body=body)
    array_notes = lF.read_file()
    for i in array_notes:
        if data.Note.Note.get_id(note) == data.Note.Note.get_id(i):
            data.Note.Note.set_id(note)
...
def show(txt):
    ...
    if array_notes:
        if txt == "all":
            ...
            for i in array_notes:
                print(data.Note.Note.map_note(i))

        elif txt == "ID":
            for i in array_notes:
                print("ID: ", data.Note.Note.get_id(i))
            id = input("\nВведите id заметки: ")
            flag = True
            for i in array_notes:
                if id == data.Note.Note.get_id(i):
                    print(data.Note.Note.map_note(i))
                    flag = False
            if flag:
                print("Нет такого ID")
            ...
            flag = True
            for i in array_notes:
                date_note = str(data.Note.Note.get_date(i))
